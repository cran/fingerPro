...
double correct_sem(double avg, double dev, int n)
{
	if( n > 1 )
	{
...
double correct_sd(double avg, double dev, int n)
{
...
Rcpp::DataFrame unmix_c_lvp(SEXP sources, SEXP mixtures, int variability, int iter=100, int resolution=100, int rng_init=123456)
{
	...
	Rcpp::DataFrame dfp = Rcpp::DataFrame(mixtures);
	...
	for( int i = 1 ; i < dfp.size() ; i++ )
	{
		...
		{
			name = "" + col_names[i];
			name2 = "" + col_names[i + dfp.size() - 1];
			stop("Column " + name2 + " must match " + name + " in source data frame");
		}
	}
...
//		namevec.push_back("Dw "+d[i]);
	...
	for( i = 0 ; i < points ; i++ )
	{
		...
		for( ii = -1 ; ii < iter ; ii++ )
		{
			// screen
//			printf("\r                                            \r");
//			printf("> Analyzing %d of %d mixtures... %.1f%c",
...
//			fflush(stdout);
			...
			for( k = 0 ; k < vars ; k++ )
			{
				for( l = 0 ; l < nsource ; l++ )
				{
					source_c[l][k] = correct_sem(source[l][k], source_d[l][k], source_n[l]);
				}
			}
